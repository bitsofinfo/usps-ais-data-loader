...
import org.bitsofinfo.util.address.usps.ais.loader.local.LocalDataLoader;
...
public class LocalDataLoaderTest extends AbstractJUnit4SpringContextTests {
	...
	public void testDataLoader() throws Exception{ 
		...
		//extractedDirs.add(new File("/tmp/z4trans"));
		//List<FileHandle> handles = reader.createHandles(extractedDirs);
		
		
		LoaderJob job = dataLoader.createJob("LocalDataLoaderTest", handles, uspsDataStore);
		dataLoader.load(job);
